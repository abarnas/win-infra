...
#Get-CACertificateDatabase -CertificationAuthority "cisinfra.local\cisinfra-ADC1-CA" -IncludeBinaryCertificate -Disposition 16 
Get-CACertificateDatabase -CertificationAuthority "cisinfra.local\cisinfra-ADC1-CA" -IncludeBinaryCertificate | ForEach-Object { $_.BinaryCertificate | Out-File "$($_.IssuedCommonName).cer" -Encoding default }